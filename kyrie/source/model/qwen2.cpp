...
base::Status Qwen2Model::init(base::DeviceType device_type) {
  ...
  if (device_type_ == base::DeviceType::kDeviceCPU) {
    kernel::sin_cos_cache_calc_cpu(config_->head_size_, config_->seq_len_,
                                   get_buffer(BufferType::kSinCache).ptr<float>(),
                                   get_buffer(BufferType::kCosCache).ptr<float>());
  } else {
    CHECK_NE(cuda_config_, nullptr);
    kernel::sin_cos_cache_calc_cu(config_->head_size_, config_->seq_len_,
                                  get_buffer(BufferType::kSinCache),
                                  get_buffer(BufferType::kCosCache), cuda_config_->stream);
  }
...
void Qwen2Model::init_mem() {
  ...
  CHECK(insert_buffer(BufferType::kSinCache, sin_cache));
  CHECK(insert_buffer(BufferType::kCosCache, cos_cache));

  CHECK(insert_buffer(BufferType::kInputTokens, input_tokens));
  CHECK(insert_buffer(BufferType::kInputEmbeddings, input_embeddings));

  tensor::Tensor rms_output(base::DataType::kDataTypeFp32, config_->dim_, true, alloc);
  CHECK(insert_buffer(BufferType::kOutputRMSNorm, rms_output));
  CHECK(insert_buffer(BufferType::kOutputMHA, rms_output));
  CHECK(insert_buffer(BufferType::kW2Output, rms_output));
  CHECK(insert_buffer(BufferType::kFFNRMSNorm, rms_output));
  ...
  CHECK(insert_buffer(BufferType::kW1Output, w1_output));
  CHECK(insert_buffer(BufferType::kW3Output, w3_output));
  ...
  CHECK(insert_buffer(BufferType::kKeyCache, key_cache));
  CHECK(insert_buffer(BufferType::kValueCache, value_cache));

  // Wq query output
  tensor::Tensor query(base::DataType::kDataTypeFp32, config_->dim_, true, alloc);
  CHECK(insert_buffer(BufferType::kQuery, query));

  // Pos tensor
  tensor::Tensor pos_tensor(base::DataType::kDataTypeInt32, 1, true, alloc_cpu);
  CHECK(insert_buffer(BufferType::kInputPos, pos_tensor));

  // Attention output
  tensor::Tensor attn(base::DataType::kDataTypeFp32, config_->head_num_, config_->seq_len_, true,
                      alloc);
  CHECK(insert_buffer(BufferType::kScoreStorage, attn));
  CHECK(insert_buffer(BufferType::kAttnOutput, query));
  ...
  if (device_type_ == base::DeviceType::kDeviceCUDA) {
    tensor::Tensor forward_output_cpu(base::DataType::kDataTypeFp32, config_->vocab_size_, true,
                                      alloc_cpu);
    CHECK(insert_buffer(BufferType::kForwardOutputCPU, forward_output_cpu));
  }

  CHECK(insert_buffer(BufferType::kForwardOutput, forward_output));
}
...
void Qwen2Model::attention_rms(int32_t layer_idx, const tensor::Tensor& input) const {
  CHECK(qwen_layers_ != nullptr);
  // attn rmsnorm
  tensor::Tensor rmsnorm_output = get_buffer(BufferType::kOutputRMSNorm);
...
void Qwen2Model::attention_qkv(int32_t layer_idx, const tensor::Tensor& pos_tensor) const {
  CHECK(qwen_layers_ != nullptr);
  // kv cache
  tensor::Tensor query = this->get_buffer(BufferType::kQuery);
  ...
  auto rmsnorm_output = get_buffer(BufferType::kOutputRMSNorm);
  ...
  STATUS_CHECK(qwen_layers_->rope_layer_->forward(
      query, key, pos_tensor, get_buffer(BufferType::kSinCache),
      get_buffer(BufferType::kCosCache), tensor::Tensor{}));
}
...
void Qwen2Model::attention_mha(int32_t layer_idx, const tensor::Tensor& pos_tensor) const {
  CHECK(qwen_layers_ != nullptr);
  // mha
  tensor::Tensor key_cache = get_buffer(BufferType::kKeyCache);
  // VAL = [val1,val2,...val t]
  // output @ VAL = 最终的结果
  tensor::Tensor val_cache = get_buffer(BufferType::kValueCache);

  tensor::Tensor mha_output = get_buffer(BufferType::kOutputMHA);
  tensor::Tensor score_storage = get_buffer(BufferType::kScoreStorage);
  tensor::Tensor query = this->get_buffer(BufferType::kQuery);
  ...
  // wo @ attention output
  tensor::Tensor attn_output = get_buffer(BufferType::kAttnOutput);
...
void Qwen2Model::feed_forward(int32_t layer_idx, const tensor::Tensor& input) const {
  ...
  STATUS_CHECK(
      qwen_layers_->add_layer_->forward(input, get_buffer(BufferType::kAttnOutput), input));

  // ffn rmsnorm
  tensor::Tensor ffn_norm_output = get_buffer(BufferType::kFFNRMSNorm);
  ...
  // w1
  tensor::Tensor w1_output = get_buffer(BufferType::kW1Output);
  ...
  // w3
  tensor::Tensor w3_ouput = get_buffer(BufferType::kW3Output);
  ...
  // w2
  tensor::Tensor w2_output = get_buffer(BufferType::kW2Output);
...
op::EmbeddingOutput Qwen2Model::embedding(const std::vector<int>& tokens) const {
  auto input_tokens = get_buffer(BufferType::kInputTokens);
  auto input_embeddings = get_buffer(BufferType::kInputEmbeddings);
...
void Qwen2Model::cls_logits(const tensor::Tensor& input) const {
  ...
  tensor::Tensor forward_output = get_buffer(BufferType::kForwardOutput);
