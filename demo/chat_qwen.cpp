...
class ChatAssistant {
 public:
  ...
  bool init() {
    try {
      model_ = std::make_unique<model::Qwen2Model>(base::TokenizerType::kTokenizerBpe, tokenizer_path_,
                                                   model_path_, false);

      auto init_status = model_->init(base::DeviceType::kDeviceCUDA);
      if (!init_status) {
        LOG(ERROR) << "模型初始化失败: " << init_status.message();
        return false;
      }
      return true;
  ...
  std::string generate(const std::string& prompt, int max_length) {
    ...
    tensor::Tensor pos_tensor = model_->get_buffer(model::BufferType::kInputPos);
