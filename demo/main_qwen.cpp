...
int32_t generate(const model::Qwen2Model& model, const std::string& sentence, int total_steps,
  ...
  tensor::Tensor pos_tensor = model.get_buffer(model::BufferType::kInputPos);
...
int main(int argc, char* argv[]) {
  ...
  model::Qwen2Model model(base::TokenizerType::kTokenizerBpe, tokenizer_path,
    checkpoint_path, false);
  auto init_status = model.init(base::DeviceType::kDeviceCUDA);
  if (!init_status) {
    LOG(FATAL) << "The model init failed, the error code is: " << init_status.code();
  }
